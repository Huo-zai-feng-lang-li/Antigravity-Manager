...
*   **版本演进**:
    ...
    *   **v4.8.29 (2026-09-17)**:
        -   **[详情弹窗滚动加平滑过渡动画]**:
            ...
            -   **性能确认**: 历史消息默认折叠（`isHistoryOpen`），默认不渲染大量 DOM；工具调用/思考过程也折叠。报文多不会卡。
    *   **v4.8.28 (2026-09-17)**:
    ...
    *   **v4.8.23 (2026-09-17)**:
        -   **[流量日志多模态图片完整预览修复（后端计数兼容 JSON 转义斜杠）]**:
            ...
            -   **修复**: 计数前缀去掉末尾斜杠（`data:image`、`"media_type":"image` 等），同时兼容 `/` 与 `\/` 两种写法。含图且 ≤10MB 的报文现按原策略完整保留，对话视图可正常渲染缩略图并点击放大。
            -   **UI 微调**: 详情弹窗 tab 栏（对话视图/原始报文）与下方内容之间补间距；会话标题列文字颜色由绿色改为与模型列一致的蓝色。
    *   **v4.8.22 (2026-09-17)**:
        -   **[黑名单/白名单拦截自定义文案在客户端正常显示]**:
            -   此前对流式对话请求（/v1/responses、/v1/chat/completions）拦截时返回 403 + JSON，客户端在流模式下不读该 body，兜底显示 "Invalid API key"，导致自定义拦截文案看不到。现改为：流式请求被拦时回 200 + SSE error 事件，把自定义拦截文案推进流内；非流式请求仍返回 403 JSON。
            -   性能：仅多读一次 Accept 头与 path（纳秒级），不读请求体、不加锁、不新增热路径磁盘 IO。
    *   **v4.8.21 (2026-09-17)**:
        -   **[详情视图切换器去掉重复图标]**:
            -   "对话视图/原始报文"切换按钮此前是 lucide 图标＋文案里的 emoji（💬/📄）叠在一起，视觉上像两个图标；现去掉文案 emoji，只保留统一风格的 lucide 图标。
    *   **v4.8.20 (2026-09-17)**:
        -   **[会话标题自动刷新 + 列颜色改绿色]**:
            -   **标题不再需要手动刷新**: 客户端生成会话标题是对话完成约数秒后的异步独立请求，桌面端此前只靠事件监听、收不到这次更新；现改为所有模式每 5 秒自动轮询列表，标题落库后几秒内自动出现。
            -   **会话标题列颜色改为绿色**，在深色/浅色主题下均清晰可辨。
            -   **图片显示说明**: 含图报文在 v4.8.19 已改为完整保留（≤10MB 不截断），经对照验证前端能正确识别完整 base64 并渲染缩略图；旧版本已截断损坏的历史图片无法恢复，需重新发送一条带图片的对话验证。
    *   **v4.8.19 (2026-09-17)**:
    ...
    *   **v4.8.14 (2026-09-17)**:
        -   **[流量日志会话标题提取与用户Token横向滚动条]**:
            -   **会话标题列正确提取**: 修复 /v1/responses 协议下代理聚合响应 `{content:"...",usage:{...}}` 格式未被识别，导致会话标题列空白；同时标题生成关键词对齐后端 TITLE_KEYWORDS，覆盖 Claude Code 等客户端的 "write a 5-10 word title" 等措辞。
            -   **用户Token页横向滚动条彻底消除**: 根容器去掉 max-w-7xl 宽度上限、表格加 w-full，7 列在宽屏下占满可用空间不再溢出。
    *   **v4.8.13 (2026-09-17)**:
        -   **[对话视图与用户Token页三处UI修正]**:
            -   **截断多模态图片不再出蓝卡片**: 被截断、无图数据的多模态图片（kind=truncated）不再渲染蓝色虚线占位卡片，避免与正文 `[图片: mime]` 文本双重占位；真图（url/base64）缩略图与点击放大功能不变。
            -   **用户Token页横向滚动条**: 表格容器加 `min-w-0`，修复 flex 子项被表格内容撑破导致整页出现横向滚动条的问题。
            -   **"旧版本"误报文案修正**: 请求体超长被头尾截断时，前端不再误报"该日志产生于旧版本"，改为"该请求体超过日志长度上限已做头尾截断，中间部分被省略，用户提问可能不完整"。
    *   **v4.8.12 (2026-09-17)**:
        -   **[深色模式 UI 修复] 批量清除 daisyUI 主题色透明度修饰符导致的浅色叠层**:
            -   **根因**: daisyUI 5 将 `base-100/200/300` 定义为 `oklch()` 函数值，Tailwind 3.4 的 `/NN` 透明度修饰符无法对其注入 alpha 通道，导致 `dark:bg-base-*/NN` 规则在构建产物中不生成；深色模式下实际生效的是前一条无条件的 `bg-gray-50/50` 浅灰半透明，叠在深色卡片上发白发雾。
            -   **修复范围**: 账号详情/错误/设备指纹对话框头部、账号卡片 loading 遮罩、对话视图消息条与思考块、CliSyncCard、OpenCodeSyncModal、熔断器、配额模型置顶、配额保护、智能预热、批量导入弹窗、API 代理配置区等 13 个组件共 23 处；统一去掉主题色上的 `/NN`，改用不透明 `dark:bg-base-100/200/300`。
            -   **多语言补全**: 统计页"解除封禁"按钮文案同步补齐 ja/ko/es/pt/ru/vi/tr/ar/my/zh-TW 共 10 个语言文件。
    *   **v4.8.11 (2026-09-17)**:
        -   **[安全监控统计页修复] 已封禁 IP 补上「解除封禁」操作入口**:
            -   **问题**: 访问排行中已封禁（is_blocked=true）的 IP，操作列仅显示静态"已封禁"红色标签，没有任何解除封禁按钮；当黑名单管理页又因内存/DB 不同步而看不到该记录时，用户看到 IP 被封却无法自助解封。
            -   **修复**: 将该静态标签替换为可点击的红色"解除封禁"按钮，点击调用后端 `remove_ip_from_blacklist`，成功后即时将该行 `is_blocked` 置为 false；操作成功/失败均有轻提示。
    *   **v4.8.8 (2026-09-17)**:
        -   **[API 监控看板重大升级] 详情抽屉新增「对话视图」与「原始报文」双视图模式**:
            -   **双视图一键切换**: 在请求详情抽屉顶部引入分段药丸切换器，默认呈现结构化清晰的「💬 对话视图」，并保留「📄 原始报文」模式供排查网络传输细节。
            -   **全协议对话解析引擎 (`logPayloadParser`)**: 智能统一解析 OpenAI、Claude、Gemini 各协议的流式与非流式请求及响应 Payload，精准提纯 System 设定、User 提问、思考过程（Thinking / Reasoning）、工具与函数调用（Tool Calls & Parameters）、模型回答及错误响应。
            -   **截断与残缺 JSON 容错恢复**: 针对反代中间层或上游长文本截断导致的非标准/残缺 JSON 报文，内置括号自动闭合与正则匹配降级抢救机制，确保复杂报文前端渲染永不崩溃。
            -   **模块化对话卡片组件 (`ConversationView`)**: 采用模块化卡片布局呈现对话流，支持深度思考链默认折叠与独立展示、工具入参格式化高亮、各消息块一键独立复制、多轮对话时间流展开以及空数据/异常提示。
            -   **全语系国际化对齐**: 同步在简体中文（`zh.json`）、繁体中文（`zh-TW.json`）与英文（`en.json`）中补全视图切换与卡片标识词条。
    *   **v4.8.7 (2026-09-14)**:
        -   **[深色模式 UI 修复] 流量日志/用户Token/设置三处浅色背景修正**:
            -   **问题根因**: daisyUI 5 主题色（base-100/200）叠加 Tailwind 透明度修饰符（/30 /50 /95）后，dark: 变体在构建产物中不生成有效规则，静默 fallback 到浅色 bg-gray-XX，导致深色模式下三处背景与整体脱节。
            -   **流量日志顶部工具栏**: 由中灰半透明改为深色 base-200，与下方分页栏一致。
            -   **用户 Token 列表表头与行 hover**: 表头由浅灰改为深色 base-200；行 hover 由近白改为深色 base-200，深色下表头/行/卡片层次清晰。
            -   **设置页顶部吸顶栏**: 由白色半透明改为深色 base-100；浅色模式保留毛玻璃半透明效果。
    *   **v4.8.6 (2026-09-14)**:
        -   **[Token 管理页与表单控件修复] 刷新反馈与输入框内边距恢复**:
            -   **刷新按钮点击必有反馈**: 手动点击刷新时，除列表重新拉取最新数据外，完成后弹出"Token 列表已刷新"轻提示（数据无变化时也能感知刷新已执行）；失败时仍显示错误提示。
            -   **全程序输入框/下拉框/文本框内边距恢复**: 修复 daisyUI 组件默认 padding 被 Tailwind preflight 清零导致的文字紧贴边框问题；按 daisyUI 默认恢复（输入框左右 12px、下拉框右端 28px 保留下拉箭头空间、文本框上下 8px），同时补回文本框边框。
    *   **v4.8.5 (2026-09-14)**:
